...
// File @confluxfans/contracts/token/CRC1155/extensions/ICRC1155Enumerable.sol@v0.2.7
...
// File @confluxfans/contracts/token/CRC1155/extensions/CRC1155Enumerable.sol@v0.2.7
...
// File contracts/lib/GranularRoles.sol
...
// File contracts/lib/ConfigManager.sol
...
contract ConfigManager {
	...
	Config.Deployment internal deployment;
	Config.Runtime internal runtime;

	event PermanentURIGlobal();

	constructor() {
		runtime = Config.Runtime("", true, true, 0, address(0));
	}
...
// File contracts/ERC1155NFTCustom.sol
...
contract ERC1155NFTCustom is CRC1155Enumerable, GranularRoles, ConfigManager {
	...
	constructor(
		Config.Deployment memory deployment,
		Config.Runtime memory runtime,
		address owner
	) ERC1155(runtime.baseURI) {
		setInitial(deployment);
		setRuntime(runtime);
		_initRolesWithMsgSender(owner);
	}

	function setURI(string memory _newURI) public onlyRole(ADMIN_ROLE) {
		require(runtime.metadataUpdatable, "NFT: Token uris are frozen globally");
		_setURI(_newURI);
	}
	...
	function burn(
		...
		require(deployment.tokensBurnable, "NFT: tokens burning is disabled");
		_burn(user, id, value);
	}
	...
	function transferByOwner(
		...
		require(runtime.tokensTransferable, "NFT: Transfers by owner are disabled");
	...
	function uri(uint256 _id) public view override returns (string memory) {
		if (bytes(_tokenURIs[_id]).length > 0) {
			if (bytes(runtime.baseURI).length > 0) {
				return string(abi.encodePacked(runtime.baseURI, _tokenURIs[_id]));
			} else {
	...
	function royaltyInfo(uint256 tokenId, uint256 salePrice) external view returns (address, uint256) {
		return (runtime.royaltiesAddress, (runtime.royaltiesBps * salePrice) / ROYALTIES_BASIS);
	}
			...
			bytes(
				string(
					abi.encodePacked(
						...
						runtime.royaltiesBps.toString(),
						// solium-disable-next-line quotes
						', "fee_recipient": "', // solhint-disable-line
						uint256(uint160(runtime.royaltiesAddress)).toHexString(20),
						// solium-disable-next-line quotes
	...
	function _exists(uint256 _tokenId) internal view virtual returns (bool) {
		return tokenByIndex(_tokenId) > 0;
	}
}
